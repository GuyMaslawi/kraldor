 ...
 * (spies + spy weapons) scaled by the intelligence upgrade (+10%/level) plus
 * any extra percentage-point bonuses (e.g. an attacker's hero spy % and active
 * guild spy spell). A mission succeeds when the attacker's value strictly
 * exceeds the defender's.
 */
...
export interface CombatPowerBreakdown {
  /** Base power before any multiplier: soldiers + relevant weapons. */
  base: number;
